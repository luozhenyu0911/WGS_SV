...
def split_cmd(cmd_file,num_of_cores,num_of_cmd):
    with open(cmd_file, 'r') as f:
        ...
        for i, cmd_list in enumerate(part_list):
            # print(cmd_list, i)
            delete_file(f"work.{str(i+1)}.sh")
            with open(f"work.{str(i+1)}.sh", "w") as f:
                ...
                for index in range(0, len(cmd_list), num_of_cmd):
                    ...
                    print(f"echo 'Processing chunk {num} of {len(range(0, len(cmd_list), num_of_cmd))} ...'\n", file=f)
                    print(f'echo "start = $(date)"', file=f)
...
if __name__ == '__main__':
    split_cmd(args.all_cmd,int(args.Number_of_cores),int(args.Number_of_cmd))